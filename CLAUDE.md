...
| **Pin `ndkVersion` in EVERY module with native code** | **`29.0.14206865`** | **AGP's strip task reads the PACKAGING module's pin.** Without it AGP wants its default NDK, and when that NDK is absent **the APK ships native libraries UNSTRIPPED, with no error.** Measured on this box, all four emulator libraries: **ARMSX2 464.0 -> 27.7 MB (-94.0%), azahar 438.2 -> 37.6 (-91.4%), Cemu 260.3 -> 44.9 (-82.8%), melonDS 75.7 -> 9.6 (-87.3%).** **Unstripped they total 1,238 MB against 120 MB stripped** — a bound on what a packed binary would carry if the strip failed. **The reduction is not a constant**: Rust `librashader` strips only 22-28%, and prebuilt ANGLE arrives already stripped. XenDroid pins the same version string independently. |
