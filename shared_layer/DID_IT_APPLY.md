...
| **15** | **THE GUEST IGNORES IT. The host applies the setting faithfully and the guest declines** | XenDroid | `internal_display_resolution` *"only tells the game what the TV is. **Most titles render at a fixed size and ignore it, so the picture usually will not change.**"* **Nothing is broken** — the setting reaches the guest, and the guest is free to disregard it |
| **14** | **A SUBSTITUTED VALUE. The request was accepted and the system granted a DIFFERENT one** | XenDroid | *"A block size we did not ask for silently changes the drain rate."* **Endemic to graphics and audio APIs**, where you request a format, buffer size or present mode and receive a supported one. **Silent by design — the API is behaving correctly** |
...
| **15** | **Not detectable from the host side at all** — the host did its job. **The only evidence is the guest's output.** The mitigations are editorial: say what the setting is NOT, **name the setting that DOES work**, and expose that one. XenDroid did all three in two commits |
| **14** | **Compare what you GOT against what you ASKED for, every time, and COUNT the difference.** Not an assert — a counter, because the substitution is legal. XenDroid publishes `stat_unexpected_frames_` from its realtime callback for exactly this |
