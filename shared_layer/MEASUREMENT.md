...
| **A POSITIVE CONTROL ON THE WRONG CHANNEL PROVES NOTHING.** `adb shell` is uid 2000 in a different SELinux context from the app: a sysfs file readable from the shell may not be readable by the app, **and vice versa** — this device's shell can read tracefs and cannot enable events. **Use `run-as <package>` when the question is about the app** | rpcsx `docs/arm64/codegen.md` |
| **GENERALISED 2026-08-26: a "channel" is any EXECUTOR, not just a uid.** THREE instances, two of them mine in one session: **uid and SELinux context** (`adb shell` against the app); **environment** (`which ccache` in Git-Bash returned nothing while `/usr/bin/ccache` exists in WSL on the same machine); and **REGEX ENGINE** — a `bug_class_sweep` pattern was validated with Python's `re`, which accepts a PCRE lookahead, while the tool runs it through `git grep -E`, which rejects it. **The detector returned zero for every fork and the self-test said OK.** > **Validate with the thing that will RUN it.** | `research_log/20260826_1710_*`, `20260825_2035_*` |
| **ESTABLISH REACH BEFORE OPTIMALITY.** Two sections of correct analysis were written about a branch that never executes; the value came from a hardcoded default. **"Reading a function and reasoning about its logic is not evidence that it runs."** The tell was cheap: enable the change and read the log for the warning it should have printed | same |
...
| **COMPUTE THE CEILING AND COMPARE IT TO THE NOISE FLOOR.** A function's total share of process work **is** the maximum possible win. azahar rejected a pipeline-lookup shortcut because `ShaderDiskCache::GetPipeline()` sat **below 1% of process work while trace variance was larger than the entire function** — no implementation could have produced a readable result. **Sibling of the applicability count, on a different axis: applicability asks whether the pattern occurs, the ceiling asks whether the site is big enough to matter if it does.** Both need a profile and no build | `research_log/20260824_2305_*` |
...
| **Capture BEFORE force-stopping or clearing the log.** A classifier over an uncleared log can classify a previous run |
| **A LOG-STORMING FAILURE ERASES WHAT PRECEDED IT.** 24,459 messages in 655 ms evicted the guest fault that came before them, twice — *"the failure mode destroyed its own diagnostic context"*. **Raise the buffer or filter at the source BEFORE reproducing one**; this is not operator error, the failure does the clearing | rpcsx `docs/arm64/spin.md` | same; `research_log/20260825_1810_*` |
...
| **THERMAL DRIFT IS WORTH ~12% ON IDENTICAL WORK** — measured in one session as the device went from **30 C to 68 C**. **That is larger than almost every effect in `DEVICE_QUEUE.md`**, so two arms starting tens of degrees apart are measuring temperature. It is why the fleet's preflight notes — *"equal 40 C starts"*, *"both arms from a 34.7 C preflight"*, *"ON was handicapped by a 4 C hotter start"* — are guarding a real confound | rpcsx `docs/arm64/spin.md` |
...
| **A NEGATIVE CONTROL THAT DOES NOTHING LOOKS EXACTLY LIKE ONE THAT WORKS.** A ~1.78 W attribution was withdrawn because its "stopped" arm ran `am force-stop` on a package name that does not exist — the APK is named for its build variant, the `applicationId` differs — so **both arms were the same configuration.** `am force-stop` on a missing package **exits zero and prints nothing.** **Verify the stop with `pidof` against the real `applicationId`** | rpcsx `docs/arm64/instruments.md` |
| **For a CONTENDED atomic the cost is WHICH LINE it touches, not HOW IT IS SPELLED.** A per-reservation LSE redesign was struck through on that ground, and the contention was then removed by fixing a backoff constant instead. **`+lse` changes the spelling** — so if it moves anything it will be in atomic-dense UNCONTENDED code | rpcsx `docs/arm64/memory-model.md` |
