...
| **Frame pacing and the two-screen presenter** | **policy owned; PROPAGATE, not a build** | `app/shell` `FramePacing.kt` + **12 tests**. Four present modes incl. host-driven vsync, mode-dependent queue depth, **a marker per screen**, Screen-2 draws on change, and frame generation gated on declared activity | **Cemu is the incumbent and nothing is converted or measured** |
| **Artifact store** | **policy owned; no store exists** | `app/shell` `ArtifactStore.kt` + **16 tests**, plus `BlockCacheKey.kt`'s `OptionsSentinel` and `PersistentBlockKey`. Two tiers, 6 distinct verdicts, content-addressed sharded paths, and the four warm-up rules xenia paid for | **no native store, no fork converted, nothing measured** |
